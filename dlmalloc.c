...
int pmalloc_load_heap_file(char *filename)
{
...
int pmalloc_store_heap_file(char *filename)
{
...
static void *osMoreCore(int size)
{
    ...
    if (size > 0)
    {
        if(allocated_space + size > EHEAP_SIZE) //It is impossible, we are running out 256MB
        {
            return (void *) MFAIL;
        }
        char *retptr =  (char*)(EHEAP_START_ADDRESS) + allocated_space;// Start Address
        allocated_space += size; //Move Forward
        return retptr;
    }
    else if (size < 0)
    {
